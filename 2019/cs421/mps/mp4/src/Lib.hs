...
istackSwap (ia:ib:is) = Just $ ib:ia:is
...
istackRot (ia:ib:ic:is) = Just $ ic : ia : ib : is
istackRot _             = Nothing
...
printStack (istack, dict, out) = (istack, dict, aux istack out)
    where
        aux istack' out' = (unwords $ map show $ reverse istack') : out'
...
--- ### Conditionals
transIf :: Transition -> Transition -> (ForthState -> ForthState)
...
cstackThen (("else", kelse):("if", kif):(c, kold):cstack) =
    Just ((c, knew):cstack) where knew = (transIf kif kelse) . kold
cstackThen (("if", kif):(c, kold):cstack) =
    Just ((c, knew):cstack) where knew = (transIf kif id) . kold
cstackThen _ = Nothing

--- ### Indefinite Loops
transWhileLoop :: Transition -> (ForthState -> ForthState)
transWhileLoop kloop kstack =
    let (i:is, d, o) = kloop kstack
    in
        if i /= 0 then
            (is, d, o)
        else
            transWhileLoop kloop (is, d, o)

# reference: https://github.com/shaohua0116/VAE-Tensorflow/blob/master/demo.py
...
def take_digit(new_label, mode, digit):
    if mode == 'same':
        p = np.where(new_label == digit)
        random_pick = np.random.choice(p[0], 2, False)
        return (random_pick[0], random_pick[1])
    else:
        p = np.where(new_label == digit)
        index_1 = np.random.choice(p[0], 1, False)
        new_p = np.array(list(set(np.arange(new_label.shape[0])) - set(p[0])), dtype=int)
        index_2 = np.random.choice(new_p, 1, False)
        return (index_1[0], index_2[0])

trans = np.arange(10)
new_label = mnist.test.labels @ trans
new_image = mnist.test.images
# Train the model
...
for digit in range(10):
    imgs = []
    a, b = take_digit(new_label, 'different', digit)
    image_a = new_image[a]
    image_b = new_image[b]
    i_a = model.transformer(image_a.reshape((1, 784)))
    i_b = model.transformer(image_b.reshape((1, 784)))
    diff = i_b - i_a

    for i in range(9):
        imgs.append( model.generator((i_a + i / 8 * diff)).reshape((28,28)) )
        ax[digit, i].axis('off')
...
for digit in range(10):
    imgs = []
    a, b = take_digit(new_label, 'same', digit)
    image_a = new_image[a]
    image_b = new_image[b]
    i_a = model.transformer(image_a.reshape((1, 784)))
    i_b = model.transformer(image_b.reshape((1, 784)))
